...
HTML
  ...
  logs)
    if [[ "${*: -1}" == "jellyfin" ]]; then
      if [[ "$recreated" == "1" ]]; then
        cat <<'LOG'
[INF] Loaded assembly Jellyfin.Plugin.Kinopoisk, Version=10.11.0.2, Culture=neutral, PublicKeyToken=null from /config/plugins/КиноПоиск_10.11.0.2/Jellyfin.Plugin.Kinopoisk.dll
[INF] Loaded assembly KinopoiskUnofficialInfo.ApiClient, Version=1.0.0.0 from /config/plugins/КиноПоиск_10.11.0.2/KinopoiskUnofficialInfo.ApiClient.dll
[INF] Loaded plugin: КиноПоиск 10.11.0.2
...
[[ -d "$OLD_DIR" ]]
[[ ! -d "$PLUGIN_ROOT/КиноПоиск_10.11.0.2" ]]

bash "$PKG/install-media-core-autonomous.sh" apply --confirm > "$ROOT/apply.log"
NEW_DIR="$PLUGIN_ROOT/КиноПоиск_10.11.0.2"
[[ -d "$NEW_DIR" ]]
[[ ! -d "$OLD_DIR" ]]
python3 - "$NEW_DIR/meta.json" <<'PY'
import json
import sys
assert json.load(open(sys.argv[1], encoding="utf-8"))["version"] == "10.11.0.2"
PY
